...
//
//
int nowScreen = 1; //окошко, которое сейчас задействовано
int start = 1;     //удалять можно
int pause = 1;     //открыто ли меню паузы
		...
		for (int j = 0; j < VisB; j++) {
			if (massivVis[i][j] == 2) {
				if (fire1.laser1X + 50 >= massivEnemyX[i][j] && fire1.laser1X <= massivEnemyX[i][j] + chest_width &&
					fire1.laser1Y + 40 >= massivEnemyY[i][j] && fire1.laser1Y + 20 <= massivEnemyY[i][j] + chest_height) {
					massivVis[i][j] = 0;
		...
		for (int j = 0; j < VisB; j++) {
			if (massivVis[i][j] == 2) {
				if (fire2.laser1X + 50 >= massivEnemyX[i][j] && fire2.laser1X <= massivEnemyX[i][j] + chest_width &&
					fire2.laser1Y + 40 >= massivEnemyY[i][j] && fire2.laser1Y + 20 <= massivEnemyY[i][j] + chest_height) {
					massivVis[i][j] = 0;
					PostQuitMessage(0);
				}
			}
		...
		for (int j = 0; j < VisB; j++) {
			if (massivVis[i][j] == 2) {
				if (fire3.laser1X + 50 >= massivEnemyX[i][j] && fire3.laser1X <= massivEnemyX[i][j] + chest_width &&
					fire3.laser1Y + 40 >= massivEnemyY[i][j] && fire3.laser1Y + 20 <= massivEnemyY[i][j] + chest_height) {
					massivVis[i][j] = 0;
					PostQuitMessage(0);
				}
			}
...
void drawStatus(HDC hdc) {
	...
	char status[100];
	sprintf_s(status, "HP = %2d          DROIDS: %2d          Meteors: %2d", PlayerHP, DroidScore, meteorScore);
	TCHAR statusT[180];
...
void startMenu() {
	...
	//2 - about
	//3 - quit
	//4 - то что там с рекурсией надо было
	//5 - и конец (наверное)
	//6 - pause
//}
...
void drawPause(HDC hdc) {
	if (pause == 1) {
		...
		//Rectangle(hdc, myWindow.left + 10, myWindow.bottom / 4, myWindow.right / 8, myWindow.bottom / 3);
		...
		/*TCHAR resume[26] = L"RESUME";*/
		//TextOut(hdc, myWindow.left + 10, myWindow.bottom / 4 + 8, (LPCWSTR)resume, _tcslen(resume));
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
		...
		{
		case VK_LEFT:
			if (PlayerX <= 20 || nowScreen != 1)
				break;
			PlayerX -= 30 /*+ diff / 2*/;
			InvalidateRect(hWnd, NULL, TRUE);
			break;
		case VK_RIGHT:
			if (PlayerX >= 1000 || nowScreen != 1)
				break;
			PlayerX += 30 /*+ diff / 2*/;
			InvalidateRect(hWnd, NULL, TRUE);
			break;
		case VK_UP:
			if (defaultPos <= 1 || nowScreen != 1)
				break;
			PlayerY -= 200 + diff / 10;
			InvalidateRect(hWnd, NULL, TRUE);
			defaultPos--;
